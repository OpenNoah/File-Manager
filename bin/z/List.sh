...
if [ "$1" = "" ] ; then
  ...
  in_show="1" ; pagenum="12" ; in_init="0"
...
until [ "$quit" = "1" ] ; do
  ...
  clear ; echo -e "$title\n$lines\n$inc上下键选择,左右键翻页: 第$light$pageno$inc页,共$light`let tmp=$pages+1 ; echo $tmp`$inc页$end"
  if [ "${#in_list[@]}" = "0" ] ; then
